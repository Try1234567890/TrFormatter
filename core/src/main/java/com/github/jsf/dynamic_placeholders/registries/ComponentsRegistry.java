...
public abstract class ComponentsRegistry<C extends Component<?>> extends Registry<UName, Function<List<Parameter<?>>, C>> {

    public Optional<Function<List<Parameter<?>>, C>> getComponent(UName name) {
        return retrieve(Preconditions.parameterNotNull(name, "name"));
    }

    public Optional<Function<List<Parameter<?>>, C>> getComponent(String name) {
        Preconditions.parameterNotNull(name, "name");

        for (Map.Entry<UName, Function<List<Parameter<?>>, C>> entry : entries()) {
