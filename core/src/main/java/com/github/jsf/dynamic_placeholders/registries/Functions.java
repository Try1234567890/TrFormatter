...
public class Functions extends ComponentsRegistry<Function> {
    ...
    public static Functions newCondition(UName name, java.util.function.Function<ComponentsInfo, Function> function) {
        Preconditions.parameterNotNull(function, "function");
        getInstance().register(name, function);
        return getInstance();
    }

    public static Optional<java.util.function.Function<ComponentsInfo, Function>> getFunction(UName name) {
        return getInstance().retrieve(Preconditions.parameterNotNull(name, "name"));
    }

    public static Optional<java.util.function.Function<ComponentsInfo, Function>> getFunction(String name) {
        Preconditions.parameterNotNull(name, "name");

        for (Map.Entry<UName, java.util.function.Function<ComponentsInfo, Function>> entry : getInstance().entries()) {
